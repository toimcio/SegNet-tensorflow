...
def conv_layer(bottom, name, shape, training_state):
    ...
    out_channel = shape[3]
    with tf.variable_scope(name) as scope:
        filt = _variable_on_cpu('weights', shape=shape, initializer=_initialization(kernel_size, input_channel),
                                enc=False)
        tf.summary.histogram(scope.name + "weight", filt)
        conv = tf.nn.conv2d(bottom, filt, [1, 1, 1, 1], padding='SAME')
        conv_biases = _variable_on_cpu('biases', shape=[out_channel], initializer=tf.constant_initializer(0.0),
                                       enc=False)
...
def unravel_index(indices, shape):
    indices = tf.to_int64(tf.expand_dims(indices, 1))
    shape = tf.to_int64(tf.expand_dims(shape, 0))
    strides = tf.to_int64(tf.cumprod(shape, reverse=True, axis=1))
    strides_shifted = tf.to_int64(tf.cumprod(shape, exclusive=True, reverse=True, axis=1))
...
def up_sampling(max_values, max_indices, shape, name):
    ...
    with tf.variable_scope(name):
        ...
        pooling_index_4d = unravel_index(indices_reshape, shape)
